...
#include "../Logger/SingletonLoggerFactory.h"
#include "../Loader/ITests.h"
...
namespace Utilities
{
 ...
 inline bool TestExecutor::execute(std::shared_ptr<ITest> pTest, const std::string& name, ILog* pLog)
  {
    bool result = false;
    try
    {
  ...
  inline void TestExecutor::check(bool result, ILog* pLog)
  {
    if (result)
      pLog->write("\n  passed");
    else
