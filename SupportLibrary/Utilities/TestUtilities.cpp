...
namespace Utilities
{
	...
	public:
		...
		std::string name() { std::string result=""; return result; };
		std::string author() { std::string result = ""; return result; };;
		// provides means for host to pass a reference to its hosted resource
		void acceptHostedResource(Utilities::SingletonLoggerFactory<1, Utilities::NoLock>* pRes) {};
	};
