...
namespace MsgPassingCommunication
{
  ...
  public:
	  ...
	  std::vector<Value> dll_value(const Key& key);
	  bool containFile();
	  ...
	  void fromTestRequest(Testing::TestRequest &src);
	  size_t dlls_size();
	  std::vector<std::string> dll_name();
	  std::ostream& show(std::ostream& out = std::cout);

  private:
	  std::vector<std::string> dlls_;//Used when more than one file to be sent
	  Attributes attributes_;//Used when only one file to be sent (key: "sendingFile")
	  // name            : msgName
	  // command         : msg Command
	  // to              : dst EndPoint
	  // from            : src EndPoint
