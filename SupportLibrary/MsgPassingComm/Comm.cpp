...
// ver 2.6                                                         //
// Author: Jim Fawcett, CSE687-OnLine Object Oriented Design, Fall 2017//
//                                                                 //
// Modifier: Yuexin Yu                                     		   //
// yyu100@syr.edu                                                  //
//                                                                 //
// Modification:												   //
// Added functions to support transfering multiple files contained //
// in test request message.										   //
/////////////////////////////////////////////////////////////////////
...
{
	StaticLogger<1>::write("\n\n-------------------- " + rcvrName + " Is Dequeuing A Message --------------------");
	return rcvQ.deQ();
}
...
void Sender::start()
{
	std::function <void()> threadProc = [&]() {
		while (true)
		{
			...
			if (msg.command() == "quit")
			{
				StaticLogger<1>::write("\n\n-------------------- A Sending Thread Is Shutting Down --------------------");
				return;
			}
			StaticLogger<1>::write("\n\n-- " + sndrName + " Is On A Sending Thread: " + msg.name());
			...
			if (msg.to().address != lastEP.address || msg.to().port != lastEP.port)
			{
				connecter.shutDown();
				//connecter.close();
				StaticLogger<1>::write("\n\n-- Attempting To Connect A New Endpoint: " + msg.to().toString());
				if (!connect(msg.to()))
				{
					StaticLogger<1>::write("\n\n-- Failed to Connect...");
					continue;
				...
				{
					StaticLogger<1>::write("\n-- Successfully Connected to: " + msg.to().toString());
				}
			}
			if(msg.containFile())
			{
...
bool Sender::sendSingleFile(Message msg)
{
	...
	while (true)
	{
		if (msg.containsKey("verbose"))
		{
			...
			std::string sid = ", Thread_ID = " + conv.str();
...
bool Sender::sendFile(Message msg)
{
	...
	if (!msg.containFile())
	{
		std::cout << "\n\n-- Message Does Not Contain the Key sendingFile In sendFile(msg)...";
		return false;
	}
	auto dlls = msg.dll_value("sendingFile");
...
namespace MsgPassingCommunication
{
	...
	*/
	...
	public:
		...
		{
			std::cout << "\n-------------------- Starting A ClientHandler --------------------\n";
		}
		//----< shutdown message >-----------------------------------------

		~ClientHandler()
		{
			std::cout << "\n-------------- A ClientHandler Has Been Destroyed ---------------\n";
		}
		//----< set BlockingQueue >----------------------------------------
		...
		*/
		...
		bool receiveFile(Message msg)
		{
			...
			if (!saveStream.good())
			{
				std::cout << "\n\n-- Failed To Open Save File \"" + fileSpec + "\"";
				return false;
			}
		...
		void operator()(Socket socket)
		{
			...
			while (true)
			{
				...
				if (msg.containsKey("verbose"))
				{
					std::cout << "\n-------------------- Receiving A Message: \"" + msg.value("command") + "\"--------------------";
				}
				...
				StaticLogger<1>::write("\n\n-- " + clientHandlerName + " RecvThread Reads Message: " + msg.name());
				if (msg.command() == "quit")
					break;
			}
