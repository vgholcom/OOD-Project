...
//----< get procid attribute >-------------------------------------------

std::string Message::procid()
{
    if (containsKey("procid"))
    {
        return attributes_["procid"];
    }
    return "";
}
//----< set procid attribute >-------------------------------------------

void Message::procid(const std::string& id)
{
    attributes_["procid"] = id;
}
